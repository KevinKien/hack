...
@section('content')
 <!-- Page Heading -->
    ...
    <div class="row">
        <div class="col-lg-12">
            {{Form::open(array('url'=>'/admin/articles/edit/'.$item->id, 'class'=>'' , 'role'=>'form', 'method'=>'post'))}}
@include('layouts._messages')
                <div class="form-group">
                    <label>Tiêu đề</label>
                    {{ Form::text( "title" , Input::old( "title", $item->title ) , array( 'class'=>'form-control required' , 'placeholder'=>'Nhập tiêu đề' ) ) }}
                </div>
                <div class="form-group">
                    <label>Mô tả</label>
                    {{ Form::textarea( "description" , Input::old( "description", $item->description ) , array( 'class'=>'form-control' , 'rows'=>'2' ) ) }}
                </div>
                <div class="form-group">
                    <label>Từ khoá</label>
                    {{ Form::text( "keyword" , Input::old( "keyword" , $item->keyword) , array( 'class'=>'form-control' , 'placeholder'=>'Cách nhau bởi dấu phẩy' ) ) }}
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <div class="col-lg-5">
                        ...
                        <div style="background: #F8F8F8; margin: 0 auto; padding: 5px">
                            <img id = "imgTopic" style="max-width: 180px; max-height: 180px" src="{{$item->thumb?$item->thumb:''}}">
                        </div>
                        <div>
                            <span id="spnFileName" class="bg-info">{{$item->thumb?$item->thumb:''}}</span>
                        </div>
                    </div>
                    <input type="hidden" name="topicImg" id="topicImg" value="{{$item->thumb?$item->thumb:''}}" />
                </div>

                <div class="form-group" style="clear: both">
                    <label>Danh Mục <a data-toggle="modal" data-target="#addCate">[Thêm]</a></label><br>
                    {{ Form::select('categories[]', $allCates, $articleCategory, array('multiple', 'class'=>'selectpicker show-tick', 'title'=>'Chọn 1 hoặc nhiều', 'id'=>'cboCates')) }}
                </div>
